...
@interface FMEasyShowTextBgView()
...
@implementation FMEasyShowTextBgView
    ...
    if ([super initWithFrame:frame]) {
        ...
        if ((!(self.isShowedStatusBar||self.isShowedNavigation))) {
            ...
            if (_showTextStatus != ShowTextStatusPureText && (!ISEMPTY_S(text))) {//只要不是纯文字，其他的都需要显示图片
                self.imageView.top  = FMEasyDrawImageEdge ;
            }
        }
        ...
        if (!ISEMPTY_S(text)) {
            CGSize textSize = [FMEasyShowUtils textWidthWithStirng:text
...
{
    if (nil == _imageView) {
        CGFloat imageWH = FMEasyDrawImageWH ;
        CGFloat imageX = (self.width-FMEasyDrawImageWH)/2 ;
        CGFloat imageY = FMEasyDrawImageEdge/2 ;
