...
#import "FMEasyShowTextView.h"
#import "UIView+FMEasyShowExt.h"

#import "FMEasyShowTextBgView.h"

@interface FMEasyShowTextView()<CAAnimationDelegate>
...
@property (nonatomic,strong)FMEasyShowTextBgView *showBgView ;//用于放图片和文字的背景
...
@implementation FMEasyShowTextView
...
- (void)showViewWithSuperView:(UIView *)superView
{
    ...
    self.showBgView = [[FMEasyShowTextBgView alloc]initWithFrame:showFrame
    ...
    if (self.options.textShadowColor && self.options.textShadowColor!=[UIColor clearColor]) {
        CGFloat afterStart = self.options.textAnimationType==TextAnimationTypeBounce ? FMEasyShowAnimationTime : FMEasyShowAnimationTime/2 ;
...
//                }];
        ...
        case TextAnimationTypeBounce:
        {
            if ( (self.isShowedStatusBar || self.isShowedNavigation)) {
                ...
                [self.showBgView showStartAnimationWithDuration:FMEasyShowAnimationTime];
            }
    ...
    switch (self.options.textAnimationType) {
        ...
        case TextAnimationTypeBounce:
        {
            ...
            if ( (self.isShowedStatusBar || self.isShowedNavigation)) {
                ...
                [self.showBgView showEndAnimationWithDuration:FMEasyShowAnimationTime];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(FMEasyShowAnimationTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [self removeFromSuperview];
                });
            }
        }
                   ...
                   stauts:(ShowTextStatus)status
{
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            FMEasyShowTextView *showView = (FMEasyShowTextView *)subview ;
            [showView removeSelfFromSuperView];
        }
    }
    
    FMEasyShowTextView *showView = [[FMEasyShowTextView alloc] initWithFrame:CGRectZero];
    ...
    if (showView.showTime > FMTextShowMaxTime) {
        showView.showTime = FMTextShowMaxTime ;
    }
...
{
    //显示图片的高度。
    CGFloat imageH = self.showTextStatus==ShowTextStatusPureText ?:(FMEasyDrawImageWH + FMEasyDrawImageEdge) ;
    ...
    switch (self.options.textStatusType) {
        ...
        default:{
            ...
            if (!ISEMPTY_S(self.showText)) {
                textSize = [FMEasyShowUtils textWidthWithStirng:self.showText
                                                         font:self.options.textTitleFount
                                                     maxWidth:FMTextShowMaxWidth];
            }
            backGroundH = (textSize.height?(textSize.height+30):0) + imageH ;
            backGroundW = textSize.width?(textSize.width+40):0  ;
            
            if (backGroundW < FMEasyShowViewMinWidth) {
                backGroundW = FMEasyShowViewMinWidth  ;
            }
        } break;
    }
    ...
    switch (self.options.textStatusType ) {
        ...
        case ShowTextStatusTypeTop:
            showFrameY = NAVIGATION_HEIGHT_S + FMEasyTextShowEdge ;
            break;
        case ShowTextStatusTypeBottom:
            showFrameY = SCREEN_HEIGHT_S - backGroundH - FMEasyTextShowEdge ;
            break ;
        default: break;
    }
    //    }
