...
@interface FMEasyShowAlertView()<CAAnimationDelegate>
...
@property (nonatomic,strong)NSMutableArray<FMEasyShowAlertItem *> *alertItemArray ;
...
@implementation FMEasyShowAlertView
...
+ (instancetype)showAlertWithType:(alertShowType)type title:(NSString *)title message:(NSString *)message
{
    ...
    FMEasyShowAlertView *showView = [[FMEasyShowAlertView alloc]initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)addItemWithTitle:(NSString *)title itemType:(ShowAlertItemType)itemType callback:(alertItemCallback)callback
{
    ...
    FMEasyShowAlertItem *item = [[FMEasyShowAlertItem alloc]init];
    item.title = title ;
...
- (void)systemShow
{
    ...
    [self.alertItemArray enumerateObjectsUsingBlock:^(FMEasyShowAlertItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [[FMEasyShowUtils topViewController] presentViewController:alertC animated:YES completion:nil];
}
...
- (void)layoutAlertSubViews
{
    ...
    CGFloat actionShowAddSafeHeiht = self.alertShowType==alertShowTypeActionSheet ? kFMEasyShowSafeBottomMargin_S : 0 ;
...
{
    FMEasyShowAlertItem *item = self.alertItemArray[button.tag];
    if (item.callback) {
        item.callback(self);
    }
    [self alertWindowTap];
...
{
    FMEasyShowAlertItem *item = self.alertItemArray[index];
    ...
    UIImage *bgImage = [FMEasyShowUtils imageWithColor:[UIColor whiteColor]];
    UIImage *bgHighImage = [FMEasyShowUtils imageWithColor:[[UIColor whiteColor]colorWithAlphaComponent:0.7] ];
...
- (void)showEndAnimationWithType:(alertAnimationType)type completion:(void(^)(void))completion
{
    ...
    switch (type) {
        ...
        {
            [UIView animateWithDuration:FMEasyShowAnimationTime
                                  delay:0
...
- (void)showStartAnimationWithType:(alertAnimationType)type completion:(void(^)(void))completion
{
    ...
    switch (type) {
        case alertAnimationTypeFade:
        {
            ...
            [UIView setAnimationDuration:FMEasyShowAnimationTime];
...
- (UILabel *)alertTitleLabel
{
    if (nil == _alertTitleLabel) {
        _alertTitleLabel = [[FMEasyShowLabel alloc] initWithContentInset:UIEdgeInsetsMake(35, 30, 15, 30)];
...
- (UILabel *)alertMessageLabel
{
    if (nil == _alertMessageLabel) {
        _alertMessageLabel = [[FMEasyShowLabel alloc] initWithContentInset:UIEdgeInsetsMake(15, 30, 20, 30)];
