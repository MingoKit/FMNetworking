...
#import "FMEasyShowUtils.h"
#import "FMEasyShowTypes.h"

UIKIT_EXTERN const CGFloat FMEasyDrawImageWH ;
UIKIT_EXTERN const CGFloat FMEasyDrawImageEdge ;
UIKIT_EXTERN const CGFloat FMEasyTextShowEdge ;
UIKIT_EXTERN const CGFloat FMEasyShowViewMinWidth ;


UIKIT_EXTERN const CGFloat FMTextShowMaxTime ;//最大的显示时长。显示的时长为字符串长度成比例。但是不会超过设置的此时间长度(默认为6秒)
UIKIT_EXTERN const CGFloat FMTextShowMaxWidth ;//文字显示的最大宽度


UIKIT_EXTERN const CGFloat FMEasyShowLodingMaxWidth  ;     //显示文字的最大宽度（高度已限制死）
UIKIT_EXTERN const CGFloat FMEasyShowLodingImageEdge ;    //加载框图片的边距
UIKIT_EXTERN const CGFloat FMEasyShowLodingImageWH  ;      //加载框图片的大小
UIKIT_EXTERN const CGFloat FMEasyShowLodingImageMaxWH  ;   //加载框图片的最大宽度

UIKIT_EXTERN const CGFloat FMEasyShowAnimationTime ;//动画时间

UIKIT_EXTERN NSString *const FMEasyShowViewDidlDismissNotification;


@interface FMEasyShowOptions : NSObject
