...
#import "FMEasyShowLodingView.h"
#import "UIView+FMEasyShowExt.h"
#import "FMEasyShowTextBgView.h"
#import "FMEasyShowLabel.h"


@interface FMEasyShowLodingView()<CAAnimationDelegate>

@property (nonatomic,strong)FMEasyShowOptions *options ;
...
@implementation FMEasyShowLodingView
...
{
    UIView *showView = [FMEasyShowUtils topViewController].view ;
    if ([FMEasyShowOptions sharedFMEasyShowOptions].lodingShowOnWindow) {
                 ...
                 imageName:(NSString *)imageName
{
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            FMEasyShowLodingView *showView = (FMEasyShowLodingView *)subview ;
            [showView removeSelfFromSuperView];
        }
    }
    
    FMEasyShowLodingView *showView = [[FMEasyShowLodingView alloc] initWithFrame:CGRectZero];
...
- (void)showViewWithSuperView:(UIView *)superView
{
    ...
    switch (self.options.lodingShowType) {
        ...
        case LodingShowTypeIndicatorLeft:
            imageSize = CGSizeMake(FMEasyShowLodingImageWH, FMEasyShowLodingImageWH);
            break;
        ...
        case LodingShowTypePlayImagesLeft:
        {
            ...
            if (tempSize.height > FMEasyShowLodingImageMaxWH) {
                tempSize.height = FMEasyShowLodingImageMaxWH ;
            }
            if (tempSize.width > FMEasyShowLodingImageMaxWH) {
                tempSize.width = FMEasyShowLodingImageMaxWH ;
            }
            imageSize = tempSize ;
        }break ;
    ...
    CGFloat textMaxWidth = FMEasyShowLodingMaxWidth - (self.options.lodingShowType%2?:(FMEasyShowLodingImageWH+FMEasyShowLodingImageEdge*2)) ;//当为左右形式的时候减去图片的宽度
    ...
    if (self.options.lodingShowType%2) {
        //左右形式
        displayAreaSize.width = imageSize.width + FMEasyShowLodingImageEdge*2 + textSize.width ;
        displayAreaSize.height = MAX(imageSize.height+ FMEasyShowLodingImageEdge*2, textSize.height) ;
    }
    else{
        //上下形式
        displayAreaSize.width = MAX(imageSize.width+2*FMEasyShowLodingImageEdge, textSize.width);
        displayAreaSize.height = imageSize.height+2*FMEasyShowLodingImageEdge + textSize.height ;
    }
    ...
    self.imageView.frame = CGRectMake(FMEasyShowLodingImageEdge, FMEasyShowLodingImageEdge, imageSize.width, imageSize.height) ;
    if (!(self.options.lodingShowType%2)) {
    ...
    if (self.options.lodingShowType%2) {//左右形式
        ...
        textLabelX = 0 ;
        textLabelY = self.imageView.bottom + FMEasyShowLodingImageEdge ;
    }
    ...
    void (^completion)(void) = ^{
        switch (self.options.lodingShowType) {
            ...
            case LodingShowTypePlayImagesLeft:
            {
                ...
                self.imageView.animationDuration = FMEasyShowAnimationTime ;
...
- (void)showFadeAnimationStart:(BOOL)isStart completion:(void(^)(void))completion
{
    ...
    [UIView animateWithDuration:FMEasyShowAnimationTime animations:^{
...
- (void)showBounceAnimationStart:(BOOL)isStart completion:(void(^)(void))completion
{
    if (isStart) {
        ...
        popAnimation.duration = FMEasyShowAnimationTime ;
    ...
    bacAnimation.duration = FMEasyShowAnimationTime ;
...
- (FMEasyShowOptions *)options
{
    if (nil == _options) {
        _options = [FMEasyShowOptions sharedFMEasyShowOptions];
    }
    return _options ;
}
- (UIView *)lodingBgView
...
{
    if (nil == _textLabel) {
        _textLabel = [[FMEasyShowLabel alloc]initWithContentInset:UIEdgeInsetsMake(10, 20, 10, 20)];
